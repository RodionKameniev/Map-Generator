...
bool Street_cluster_spawn::try_to_build(
    ...
    vector<Position> all_street_points;
    ...
    for (
        ...
        Position shifted_pos =
            this->get_street()
            ->get_street_components()[i]
            .get_shifted_position();

            shifted_pos.set_on_x(
                shifted_pos.get_on_x()
                + start_of_street.get_on_x()
            );

            shifted_pos.set_on_y(
                shifted_pos.get_on_y()
                + start_of_street.get_on_y()
            );
            ...
            // Road
            if (
                dynamic_cast<Road_on_map*>(cell)
                ) {

                Road_on_map* road =
                    dynamic_cast<Road_on_map*>(cell);

                const Parameters_for_road* params =
                    road->get_road_to_be_placed();

                if (
                    params->get_is_restricted_to_cross()
                    ) {
                    return false;
                }
            }
            ...
            if (
                ...
                const Parameters_for_clutter* params =
                    clutter->get_clutter_to_be_placed();
            ...
            if (
                ...
                const Parameters_for_building* params =
                    building->get_building_to_be_placed();
            ...
            if (
                ...
                const Parameters_for_street* params =
                    street->get_street_to_be_placed();
